...
//    morphologyEx(threshold_frame, threshold_frame, MORPH_OPEN, kernel_open);
    ...
    return Point(PredcitShootArmourCenter.x,PredcitShootArmourCenter.y);
...
void DetectDafuArmor(Mat &grayImage, Mat &dstImage)
{
   ...
   vector <Point2f> DetectArmourCenter(contours.size());   //所有检测到的装甲板的中心坐标
    //float radius[contours.size()];


    //dstImage = Mat::zeros(grayImage.size(), CV_8UC3);
    //绘制轮廓图 没有子轮廓，但是有父轮廓的——装甲板的第一个特征
	//val[next,previous,First_child,Parent];
 ...
 //检测大符中心。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。
    ...
    //用装甲去匹配圆心
    float real_armour_dafuCenter_pixel_length = 80;
    for (int i = 0; i < contours.size(); i++)
    {
...
int main()
{
	Mat srcImg;
	VideoCapture cap;
	cap.open("E:\\RM\\tx2\\2019_win_dafu\\dafu\\dafu\\video\\dafu_red.avi");
